...
cd $HOME
sudo apt update
sudo apt install make clang pkg-config libssl-dev build-essential lz4 git jq ncdu bsdmainutils htop -y < "/dev/null"
...
curl -s https://networks.itn2.nibiru.fi/$CHAIN_ID/genesis > $NODE_HOME/config/genesis.json
