...
LATEST_SNAPSHOT=$(curl -s  https://server-5.itrocket.net/testnet/empeiria/ | grep -oE 'empeiria_[0-9-]+_([0-9]+)_snap\.tar\.lz4' | sort -t'_' -k3 -n | tail -1)
curl -S https://server-5.itrocket.net/testnet/empeiria/$LATEST_SNAPSHOT | lz4 -dc - | tar -xf - -C $HOME/.empe-chain
