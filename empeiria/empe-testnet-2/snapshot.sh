...
SNAPSHOT_URL="https://server-1.itrocket.net/testnet/empeiria/"
...
LATEST_SNAPSHOT=$(curl -s  $SNAPSHOT_URL | grep -oE 'empeiria_[0-9-]+_([0-9]+)_snap\.tar\.lz4' | sort -t'_' -k3 -n | tail -1)
curl -S $SNAPSHOT_URL$LATEST_SNAPSHOT | lz4 -dc - | tar -xf - -C $HOME/.empe-chain
