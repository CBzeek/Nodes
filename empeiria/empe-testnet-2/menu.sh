...
node_install() {
  ...
  sed -i -e "/^\[p2p\]/,/^\[/{s/^[[:space:]]*seeds *=.*/seeds = \"$SEEDS\"/}" \
         -e "/^\[p2p\]/,/^\[/{s/^[[:space:]]*persistent_peers *=.*/persistent_peers = \"$PEERS\"/}" $HOME/$WORK_DIR/config/config.toml

  # Set pruning
  pruning="custom"
  pruning_keep_recent="100"
  pruning_interval="19"
  sed -i -e "s/^pruning *=.*/pruning = \"$pruning\"/" $HOME/$WORK_DIR/config/app.toml
  sed -i -e "s/^pruning-keep-recent *=.*/pruning-keep-recent = \"$pruning_keep_recent\"/" $HOME/$WORK_DIR/config/app.toml
  sed -i -e "s/^pruning-interval *=.*/pruning-interval = \"$pruning_interval\"/" $HOME/$WORK_DIR/config/app.toml

  # Set minimum gas
  minimum_gas="0.0001uempe"
  sed -i "s/^minimum-gas-prices *=.*/minimum-gas-prices = \"$minimum_gas\"/" $HOME/$WORK_DIR/config/app.toml
  
  # Disable indexing
  indexer="null"
  sed -i -e "s/^indexer *=.*/indexer = \"$indexer\"/" $HOME/$WORK_DIR/config/config.toml
  
  # Enable prometheus
  sed -i -e "s/prometheus = false/prometheus = true/" $HOME/$WORK_DIR/config/config.toml
...
[Service]
User=$USER
WorkingDirectory=$HOME/$WORK_DIR
ExecStart=$(which emped) start --home $HOME/$WORK_DIR
Restart=on-failure
RestartSec=5
LimitNOFILE=65535
[Install]
